...
class Cookie
{
    ...
    public static function init(array $opts = array())
    {
        ...
        if (isset($opts['path']) && (string) $opts['path']) self::$path = (string) $opts['path'];
        if (isset($opts['prefix'])) self::$prefix = (string) $opts['prefix'];

        if (isset($opts['domain']) && (string) $opts['domain']) self::$domain = (string) $opts['domain'];
        else
        {
    ...
    public static function set($name, $value, $expire = self::PERMANENT, $httponly = false)
    {
        ...
        elseif (is_string($expire) && $expire) $expire = strtotime($expire);
        else $expire = 0;
