...
class Crypto
{
    ...
    public static function changePepper($new)
    {
        ...
        if ((empty($new)) || ( ! is_string($new))) {
           echo "You must set a pepper and it cannot change the pepper to something that isn't a string.";
        }
    ...
    public static function randomInt($min, $max)
    {
        ...
        if ($max <= $min) {
            echo 'Minimum equal or greater than maximum!';
        }

        if ($max < 0 || $min < 0) {
            echo 'Only positive integers supported for now!';
        }
    ...
    {
        if ($length < 1) {
            echo "String length must be a positive integer.";
        }
    ...
    {
        if ($numbers <= 1) {
            echo "Numbers argument is too low.";
        }

        if ($checks <= 1) {
            echo "Checks argument is too low.";
        }
    ...
    protected static function checkMCrypt()
    {
        foreach (self::$mcryptCiphers as $cipher) {
            ...
            if ( ! ($ivSize % 16) && ! ($ivSize > 0)) {
                echo 'Your MCrypt version is too old and does not support the Rijndael, Serpant or Twofish ciphers (or the MCrypt ciphers have been changed).';
            }
        }
    ...
    {
        if (empty($data)) {
            echo 'Some data is required to encrypt.';
        }
    ...
    {
        if (empty($data)) {
            echo 'Some data is required to decrypt.';
        }
        ...
        echo 'Decryption failed (likely incorrect password).';
